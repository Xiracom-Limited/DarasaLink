...
<div class=" ">
        ...
        <div class="col-lg-12">
          <div class="form-group row">  
            ...
            <div class="col-sm-9">
                ...
                <label class="radio-inline">
                     <input type="radio" name="payment_mode_fee" value="M-pesa" onclick="showMpesaForm()"> 
                      <?php echo $this->lang->line('M-pesa'); ?>
                </label>
  ...
  <form method="post" action="<?php echo site_url('mpesa_controller/process'); ?>" id="mpesaPaymentForm">
    ...
    <div class="form-group">
      <label>Amount</label>
      <input type="number" name="amount" class="form-control" value="<?php echo $total_amount; ?>" readonly>
    </div>
    <!-- Hidden fields for context -->
    <input type="hidden" name="student_id" value="<?php echo $student_id; ?>"> <!-- Replace with actual student ID -->
    <input type="hidden" name="collected_date" value="<?php echo isset($_POST['collected_date']) ? $_POST['collected_date'] : date('Y-m-d'); ?>">
    <input type="hidden" name="fee_gupcollected_note" value="<?php echo isset($_POST['fee_gupcollected_note']) ? $_POST['fee_gupcollected_note'] : ''; ?>">
    <?php foreach ($feearray as $fee_key => $fee_value) { ?>
        <input type="hidden" name="row_counter[]" value="<?php echo $fee_key + 1; ?>">
        <input type="hidden" name="student_fees_master_id_<?php echo $fee_key + 1; ?>" value="<?php echo $fee_value->id; ?>">
        <input type="hidden" name="fee_groups_feetype_id_<?php echo $fee_key + 1; ?>" value="<?php echo $fee_value->fee_groups_feetype_id; ?>">
        <input type="hidden" name="fee_groups_feetype_fine_amount_<?php echo $fee_key + 1; ?>" value="<?php echo $fees_fine_amount; ?>">
        <input type="hidden" name="fee_amount_<?php echo $fee_key + 1; ?>" value="<?php echo $amount_to_be_pay; ?>">
        <input type="hidden" name="fee_category_<?php echo $fee_key + 1; ?>" value="<?php echo $fee_value->fee_category; ?>">
        <input type="hidden" name="trans_fee_id_<?php echo $fee_key + 1; ?>" value="<?php echo isset($fee_value->trans_fee_id) ? $fee_value->trans_fee_id : 0; ?>">
    <?php } ?>
